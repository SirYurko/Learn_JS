...
// https://www.youtube.com/watch?v=AkgJWHNzLlg

// Приклад використання [Symbol.toPrimitive]

const person = {
    name: "Alex",
    age: 22,
// Метод для перетворення об'єкта в примітив залежно від підказки (hint)
    [Symbol.toPrimitive]: function (hint){ // hint: string, number, default
        console.log(hint) // Виводимо підказку (hint) у консоль для аналізу
        switch(hint) {
            case 'string':
                return this.name
            case 'number':
...
// Порожній об'єкт personList, який буде використовуватися як словник
const personList = {};

// Додаємо до personList властивість із ключем +person (перетворюється в число)
// Оператор + викликає [Symbol.toPrimitive] із підказкою "number"
personList[+person] = "Odessa";

// Виводимо об'єкт personList у консоль
console.log(personList)

// Результат: { '22': 'Odessa' }
// Ключ '22' зберігається як рядок, а значенням є "Odessa"
